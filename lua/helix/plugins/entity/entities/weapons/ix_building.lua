...
function SWEP:MakeGhostFurniture( model, pos, angle )
	...
	-- util.IsValidProp() only accepts models compiled as physics props, which
	-- excludes plain pickup-item models (e.g. models/items/hevsuit.mdl) used
	-- by deployable entity items. This ghost is a non-solid visual-only
	-- preview, so just make sure the model itself is a real, loadable model.
	...
	-- Plain white unlit material so SetColor's tint renders as a solid translucent
	-- silhouette in the schema's theme color, instead of the old wireframe grid look.
	self.GhostEntity:SetMaterial( "models/debug/debugwhite" )
	self.GhostEntity:SetColor( ColorAlpha( ix.config.Get( "color" ) or color_white, 120 ) )
...
function SWEP:Think()
	...
	if (!IsValid(owner) or !IsValid(owner:GetActiveWeapon()) or owner:GetActiveWeapon():GetClass() != "ix_building") then
		self:ReleaseGhostEntity()
		self:Remove()
		return
	end

	local furniture = owner:GetLocalVar("furniture", nil)
	local wantModel = istable(furniture) and furniture.model or nil

	if (wantModel) then
		-- Only (re)build the ghost when the desired model actually changes.
		-- The old check compared a model string against a table every tick,
		-- so it rebuilt the client prop every single frame.
		if (!IsValid(self.GhostEntity) or self.GhostEntity:GetModel() != wantModel) then
			self:MakeGhostFurniture(wantModel, Vector(0, 0, 0), Angle(0, 0, 0))
		end

		self:UpdateGhostEntity(self.GhostEntity, owner)
	else
		self:ReleaseGhostEntity()
	end
end
